  ...
  <grid :grid="features[curFeature].showtools?gridtools:grid" style="grid-template-columns: 1fr 350px; grid-template-rows: 40px 1fr 100px 70px 70px;">
    <div class="home shadow" style="background: #304156; grid-area: a; padding: 10px; position: relative;">
      <THREE class="shadow" style="position: relative;" :isDebug="isDebug">
        ...
        <component :is="curFeature && features[curFeature].tools" style="z-index: 2" :renderer="$refs.renderer" :controls="$refs.controls" ref="tools" :key="curedit.scene_id" :editor="self"/>
      </THREE>
    </div>
...
export default {
  mixins:[mixin],
  components:{...THREE, ...sidepanel, ...scenetab, ...Tools, autosave, autofill, exhibitionBox, exhibitionEntrance, UploadSource, advs},
  data(){
    return {
    curFeature: 0,
    self: this,
